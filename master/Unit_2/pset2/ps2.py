...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        newCleanTile = (math.floor(pos.x), math.floor(pos.y))
        if newCleanTile not in self.cleanTiles:
            self.cleanTiles.append(newCleanTile)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    """
    total_steps = 0
    required_clean_tiles = (width * height) * min_coverage
    ...
    for i in range(num_trials):
        ...
        while room.getNumCleanedTiles() < required_clean_tiles:
            for robot in robots:
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        # Check that the robot is not trying to leave the room
...
#     plot.
showPlot1("Time it takes 10 robots to clean 80 percent of a room", \
    "Aspect Ratio", "Times")

#
# 2) Write a function call to showPlot2 that generates an appropriately-labeled
#     plot.
showPlot2("Time it takes 2 robots to clean 80 percent of variously sized rooms", \
    "Aspect Ratio", "Times")
