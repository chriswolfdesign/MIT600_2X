...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.title("Tempature change over years")     
    pylab.xlabel("Years")
    ...
    for model in models:
        ...
        r2 = r_squared(y, yhat)
        label_text = '{0} degree model, R2:{1}'.format(len(model)-1, r2)
        pylab.plot(x, yhat, colors[len(model)], label = label_text)
        r2List.append(r_squared(y, yhat))
    pylab.legend()
    pylab.show()
